...
type Simulation = {
  ...
  counts: { activeTrips: number; speedingNow: number; pendingDecisions: number; criticalEvents: number; totalEvents: number };
  trips: LiveTrip[]; events: LiveEvent[];
  vendorRisk: Array<{ vendor: string; activeTrips: number; speedingNow: number; repeatEvents: number; highestSeverity: string }>;
};
  ...
  return <div className="monitoring-workspace">
    ...
    <section className="panel priority-alert-inbox">
      ...
      <div className="priority-alert-list">{importantEvents.length ? importantEvents.map((event) => <article className={`priority-alert-card ${severityClass(event.severity)}`} key={event.eventId}>
        ...
        <div className="priority-alert-vendor"><span>Driver · vendor</span><strong>{event.driver || "Masked driver"}</strong><small>{event.vendor} · Trip #{event.tripId}</small></div>
        <div className="priority-alert-signal"><span>Signal</span><strong>{event.speed} km/h in a {event.speedLimit} zone</strong><small>+{event.excessKph} km/h · occurrence #{event.repeatCount}</small></div>
